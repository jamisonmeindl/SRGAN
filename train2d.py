...
if __name__ == '__main__':
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for lr_batch, hr_batch in loader:
            ...
            B, C, D, H, W = hr_batch.shape
            new_shape = (B * D, C, H, W)
            hr_batch = hr_batch.reshape(new_shape)
            fake_img = fake_img.reshape(new_shape)
            netD.zero_grad()
            ...
            # new_shape = (B * D, C, H, W)
            ##
            g_loss = generator_criterion(fake_out, fake_img, hr_batch)
            g_loss.backward()
            optimizerG.step()
        ...
        torch.save(netG.state_dict(), 'base_dis3/netG_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
        torch.save(netD.state_dict(), 'base_dis3/netD_epoch_%d_%d.pth' % (UPSCALE_FACTOR, epoch))
