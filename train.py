...
if __name__ == '__main__':
    ...
    for dir_name in common_directories:
    ...
    # train_loader = DataLoader(dataset=train_set, num_workers=1, batch_size=2, shuffle=True)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for dir_name in list(common_directories):
            path1 = os.path.join(folder_path1, dir_name)
            path2 = os.path.join(folder_path2, dir_name)
            ...
            for image_name in sorted(list(common_images)):
                imagefile_hr = os.path.join(path1, image_name)
                imagefile_lr = os.path.join(path2, image_name)
                img_hr = Image.open(imagefile_hr)
                img_lr = Image.open(imagefile_lr)
                tensor_hr = transform(img_hr)
                tensor_lr = transform(img_lr)
                tensors_hr.append(tensor_hr)
                tensors_lr.append(tensor_lr)
            hr_images = torch.stack(tensors_hr, dim=0)
            lr_images = torch.stack(tensors_lr, dim=0)

            for hr_batch, lr_batch in zip(hr_images.chunk(20), lr_images.chunk(20)):
                g_update_first = True
